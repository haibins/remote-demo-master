...
const RemoteCom = loadAsyncComponent(
  () =>
    new Promise((res) => {
      const preFetchLibParam: IPreFetchLibOptions = config.value.version
        ? {
      ...
      if (config.value.cusHost) {
        preFetchLibParam.custom = {
          host: config.value.cusHost, // 基于 web-dev-server 开发中生成产物联调
          enable: enableCustom,
        };
      }
      ...
      preFetchLib(config.value.package, preFetchLibParam)
        .then((libs) => {
          ...
          const lib = config.value.libName
            ? libs?.[config.value.libName]
            : libs;
          res(lib ?? error);
        })
        .catch((err) => {
